...
const MatchingScreen = () => {
  ...
  const handleSwipe = async (targetGroupId: string, liked: boolean) => {
    ...
    try {
      ...
      console.log(
        `Swiping on group ${targetGroupId} from ${currentGroupId}, Liked: ${liked}`
      );
      ...
      setPotentialGroups((prevGroups) =>
        prevGroups.filter((group) => group.id !== targetGroupId)
      );
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <Text style={styles.emptyText}>
              No more groups to match with right now.
            </Text>
          }
          contentContainerStyle={
            potentialGroups.length === 0 ? styles.emptyListContainer : null
          }
        />
      )}
    </View>
